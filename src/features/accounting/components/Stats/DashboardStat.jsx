...
const DashboardStat = ({ label, value, trend, isPositive, icon: Icon, color, loading }) => {
  ...
  return (
    <div className="acc-card group relative overflow-hidden cursor-pointer" style={{ padding: 'var(--space-xl)' }}>
      ...
      <div className="relative z-10">
        <div className="flex justify-between items-start" style={{ marginBottom: 'var(--space-lg)' }}>
          ...
          <div className={`px-2.5 py-1 rounded-full text-[10px] font-black flex items-center gap-1 transition-transform group-hover:scale-110 ${
        ...
        <div className="space-y-1">
          <p className="text-label-xs text-acc-text-light">{label}</p>
          <h2 className="text-display-sm text-acc-text-main tracking-tight group-hover:text-acc-primary transition-colors">
            {value || "0₫"}
          </h2>
        </div>
      </div>
